...
function EYE_POS = ComputeEyePos3D(ROOT,EYE_FOLDER,EYE_DEPTH_PATH,EYE_TAG,camera_parameters_file,CALIBRATION_POINTS_NUM,output_folder,VIEW,starting_time)
%%
%% INPUT
% ROOT root folder
...
addpath function/npy-matlab-master
addpath function
...
%% LOAD FILES
% camera parameters
load([camera_parameters_file]);

% open video file
%% (* [status, cmdout] = system(['ffmpeg -y -i ', fullfile(EYE_FOLDER,'world.mp4'), ' -c:v rawvideo -pix_fmt bgr24 ', fullfile(output_folder,'world.avi')]);
%% world_video = VideoReader(fullfile(output_folder 'world.avi'));

%%[status, cmdout] = system(['ffmpeg -y -i /hmet_data/ad/2021_05_09/007/world.mp4 -c:v rawvideo -pix_fmt bgr24 ', '/home/vasha/testoutworld.avi']);
%%world_video = VideoReader(fullfile(output_folder 'world.avi')); *)
%%(* EYE.FRAME_RES = [world_video.Width world_video.Height]; *)
EYE.FRAME_RES = [960 540]

% load target data
MIN_TIME = [];
TARGET = importdata([EYE_FOLDER '/3d_calibration/marker_center.csv']);
...
TIME.DEPTH = importdata([EYE_FOLDER '/depth/timestamps.csv']);
MIN_TIME = [MIN_TIME min(TIME.DEPTH)];

if ~exist([EYE_FOLDER '/offline_data/pupil_0.csv'])
    system('cd utils');
    folder_name = [EYE_FOLDER '/offline_data'];
...
% load target data
if strcmp(EYE_TAG,'L')
    TIME.EYE = readNPY([EYE_FOLDER '/eye1_timestamps.npy']); 
    disp([EYE_FOLDER '/offline_data/pupil_1.csv'])
    EYE.DATA_RAW = importdata([EYE_FOLDER '/offline_data/pupil_1.csv']);
else
    TIME.EYE = readNPY([EYE_FOLDER '/eye0_timestamps.npy']);
    EYE.DATA_RAW = importdata([EYE_FOLDER '/offline_data/pupil_0.csv']);
end
...
% load world camera
TIME.WORLD = readNPY([EYE_FOLDER '/world_timestamps.npy']);
...
%%    
    %% LOAD DEPTH FRAME
    %depthFrame = double(readNPY([EYE_DEPTH_PATH '/depth/depth_frame_' num2str(IDX_2DEPTH(i,2),'%08.f') '.npy']));
    depthFrame = double(imread([EYE_DEPTH_PATH '/depth_frame_' num2str(DEPTH_IDX,'%08.f') '.png']));
...
save_file_name = [output_folder '/EYE_POSITION_WS'];
var_string = [];
VAR_LIST = whos;
...
save_file_name = [output_folder '/EYE_POSITION'];
...
if EYE_TAG == 'L'
    LE = EYE;
    save_file_name = [output_folder, '/EYE_POSITION_LE'];
    save(save_file_name,'LE')
end

if EYE_TAG == 'R'
    RE = EYE;
    save_file_name = [output_folder, '/EYE_POSITION_RE'];
    save(save_file_name,'RE')
end
