...
function EYE = CalibrateEye3D(ROOT,eye_flag,camera_parameters_file,CALIBRATION_POINTS_NUM,VIEW)
%%
%% INPUT
% ROOT root folder
...
% load target data
if strcmp(eye_flag,'L')
    TIME.EYE = readNPY([ROOT EYE_TAG '\eye1_timestamps.npy']);
    
    if ~exist([ROOT EYE_TAG '\pupil_1.csv'])
        system('cd utils');
        system(['python utils/parseEYE_PL_Data.py ' ROOT EYE_TAG]);
        system('cd ..');
    end
    EYE.DATA_RAW = importdata([ROOT EYE_TAG '\pupil_1.csv']);
else
    TIME.EYE = readNPY([ROOT EYE_TAG '\eye0_timestamps.npy']);
    ...
    EYE.DATA_RAW = importdata([ROOT EYE_TAG '\pupil_0.csv']);
end
...
for i = 1:CALIBRATION_POINTS_NUM
    EYE.PUPIL.TARGET_X(CLUSTER_IDX==i) = EYE.PUPIL.X(IDX(CLUSTER_IDX==i,3));
    EYE.PUPIL.TARGET_Y(CLUSTER_IDX==i) = EYE.PUPIL.Y(IDX(CLUSTER_IDX==i,3));
end
...
if VIEW
    ...
    title(['Cluster Assignments and Centroids - ' EYE_TAG ' eye'])
...
save_file_name = [ROOT EYE_TAG '\EYE_POSITION_WS'];
var_string = [];
VAR_LIST = whos;
