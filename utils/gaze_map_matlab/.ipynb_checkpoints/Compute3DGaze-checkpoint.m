function [FixPoint3D, FixPoint2D] = Compute3DGaze(ROOT,CALIB_FOLDER,EXP_FOLDER,DEPTH_FOLDER,camera_parameters_file,CREATE_VIDEO,OUT_FOLDER,VIEW)
...
TIME.DEPTH = importdata([ROOT EXP_FOLDER '/depth/timestamps.csv']);
...
load([output_folder '/BINOCULAR_CALIB'])
...
if CREATE_VIDEO
    if ~exist([OUT_FOLDER '/POST_PROCESSING'])
        mkdir([OUT_FOLDER '/POST_PROCESSING'])
    end
%     depth_video = VideoWriter([ouput_folder '/POST_PROCESSING/depth_video.mp4'],'MPEG-4');
    depth_video = VideoWriter([ouput_folder '/POST_PROCESSING/depth_video.avi']);
...
for i = 1:BIN_EYE.FixPointNUM
    ...
    if ~isnan(sum(IDX_2L(i,:)))
        try
            ...
            %% DEPTH FRAME
            %%depthFrame = double(readNPY([DEPTH_FOLDER '/depth/depth_frame_' num2str(DEPTH_IDX,'%08.f') '.npy']));
            depthFrame = double(imread([DEPTH_FOLDER '/depth_frame_' num2str(DEPTH_IDX,'%08.f') '.png']));
