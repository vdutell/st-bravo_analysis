import numpy as np
def convert_pldata_csv(folder, pldata_filename):
    ts = []
    data = []
    topics = []
    
    msgpack_file = os.path.join(folder,pldata_filename)
    ...
    #grab relavent data
    #eye_0_xy = np.array([d['location'] for d in data if d['topic']=='pupil.0'])
    eye_0_xy = np.array([d['ellipse']['center'] for d in data if d['topic']=='pupil.0'])
    eye_0_ax = np.array([d['ellipse']['axes'] for d in data if d['topic']=='pupil.0'])
    eye_0_th = np.array([d['ellipse']['angle'] for d in data if d['topic']=='pupil.0'])
    eye_0_conf = [d['confidence'] for d in data if d['topic']=='pupil.0']
    eye_0_timestamps = [d['timestamp'] for d in data if d['topic']=='pupil.0']

    #eye_1_xy = np.array([d['location'] for d in data if d['topic']=='pupil.1'])
    eye_1_xy = np.array([d['ellipse']['center'] for d in data if d['topic']=='pupil.1'])
    eye_1_ax = np.array([d['ellipse']['axes'] for d in data if d['topic']=='pupil.1'])
    eye_1_th = np.array([d['ellipse']['angle'] for d in data if d['topic']=='pupil.1'])
    eye_1_conf = [d['confidence'] for d in data if d['topic']=='pupil.1']
    eye_1_timestamps = [d['timestamp'] for d in data if d['topic']=='pupil.1']
    
    #combine into array
    eye_0 = np.array((eye_0_xy[:,0], eye_0_xy[:,1], 
                      eye_0_ax[:,0], eye_0_ax[:,1],
                      eye_0_th, eye_0_conf, eye_0_timestamps))
    eye_1 = np.array((eye_1_xy[:,0], eye_1_xy[:,1], 
                      eye_1_ax[:,0], eye_1_ax[:,1],
                      eye_1_th, eye_1_conf, eye_1_timestamps))

    #sort by ascending timestamps
    eye_0 = eye_0[:,eye_0[-1].argsort()]
    eye_1 = eye_1[:,eye_1[-1].argsort()]
    
    header = "X0,Y0,AX,AY,THETA,CONF,TIME"
    
    np.savetxt(os.path.join(folder,'eye_0.csv'), eye_0, delimiter=",", header=header)
    np.savetxt(os.path.join(folder,'eye_1.csv'), eye_1, delimiter=",", header=header)
    
#     eye0_dict = {'TIME': time0,'X0': x0_0,'Y0': y0_0,'AX': ax0,'AY': ay0,'THETA': theta0,'CONF': conf0}
#     eye0 = pd.DataFrame(EYE0)
#     eye0.to_csv(folder_test + 'pupil_0.csv', index = True, header=True)
                  
#     eye1_dict = {'TIME': time1,'X0': x1_1,'Y0': y1_1,'AX': ax1,'AY': ay1,'THETA': theta1,'CONF': conf1}
#     eye1 = pd.DataFrame(EYE1)
#     eye1.to_csv(folder_test + 'pupil_1.csv', index = True, header=True)
        
    
#### Pulled From pupil labs code ####
